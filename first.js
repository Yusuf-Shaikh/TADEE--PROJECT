...
function compute() {
  ...
  console.log("IR=", Ir, "Pfr=", Pfr, "Pr=", Pr, "Irangle=", theta, "Irmag=", Irmag);

  // 1. Inductance per phase per km in H/km
  SGMDL = Math.pow(SGMD(), 1 / N);
  Inductance = 2 * math.pow(10, -4) * Math.log(MGMD() / SGMDL);
  console.log("Inductance_per=", Inductance, "SGMDL=", SGMDL, "IMGMD=", MGMD());
  ...
  Capacitance =
    ...
    Math.log(MGMD() / SGMDC);
  console.log("Capacitance_per=", Capacitance, "SGMDC=", SGMDC);

  // 3. Inductive reactance of the line in Ohm.
  Xl = w * Inductance * l;
  console.log("Ind_Reac_per=", Xl);

  // 4. Capacitive reactance of the line in Ohm.
  Xc = 1 / (w * Capacitance * l);
  console.log("Cap_Reac_per=", Xc);
...
function radius() {
  ...
  radi = (2 * layer - 1) * d / 2;
  console.log("Layer=", layer, "Radius=", radi);
  return radi;
}
...
function Output() {
  ...
  output[9].innerText = math.round(Vs, 5);
